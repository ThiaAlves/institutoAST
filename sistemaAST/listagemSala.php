...
<?php include("nav-bar.php"); ?>
    ...
    <div class="container" style="background-color: rgba(0, 0, 0, 0.66); color: white">
        ...
        <div class="container" ><br><br>
            <div class="text-center border border-white rounded"><h3 class="pt-3">Salas cadastradas no sistema</h3>
             </div><br><br>
        </div>  
                    ...
                    if(mysqli_num_rows($resultado) > 0){
                        while($linhas = mysqli_fetch_assoc($resultado)){ ?>
                            ...
                            <tr>
                                ...
                                <td>  <?php echo listaSalas($linhas['numeroSala'], 1)?>  </td>
                                <td>  <?php echo $linhas['qtdAluno']?>  </td>
                                <td>  <?php if($linhas['status'] == 1){echo "Ativo";}else{echo "Inativo";}?>  </td>

                                <td>
                                <?php 
                                    $linkAlterar = "<a class='btn btn-primary btn-sm btn-block' href='alterarSala.php?id={$id}'>Alterar</a>";
                                    echo "{$linkAlterar}";
                                
                                    $linkExcluir = "<a class='btn btn-danger btn-sm btn-block' href='excluirSala.php?id={$id}'  onclick='solicitaExclusao()'>Excluir</a>"; 
                                    echo "{$linkExcluir}";
                                ?>
                                </td>
                            </tr>
